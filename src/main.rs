...
use clap::Parser;
use exe::{Buffer, ImageDirectoryEntry, VecPE, PE, RVA};
use goblin::mach::{constants::SEG_LINKEDIT, load_command::CommandVariant, Mach};
use sha2::Digest;
use std::{
    path::{Path, PathBuf},
    process::Command,
...
fn main() {
    ...
    if sea_config["output"].as_str().unwrap().replace("\\", "/") != expected_path {
    ...
    if !prepared_node.exists() {
        ...
        if target == Target::default() {
            ...
            if target != Target::WinArm64 && target != Target::WinX64 {
                ...
                // set prepared_node to binary (remove .tar.gz extension) after opening the archive
            ...
            Target::DarwinArm64 | Target::DarwinX64 => {
                ...
                match Mach::parse(&buffer) {
                    Ok(Mach::Binary(macho)) => {
                        if !macho.is_64 {
                            panic!("Removing code signature from 32-bit Mach-O files is not supported yet.");
                        }
    ...
    let npx_postject_cmd = Command::new("npx")
        .arg("--yes")
    ...
    if target == Target::DarwinArm64 || target == Target::DarwinX64 {
        ...
        println!("Note: you may still need to sign the binary with your apple signing certificate later.");
